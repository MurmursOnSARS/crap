			...
			render() {
			  const element = document.querySelector(`.${channelBase.sidebar.split(' ')[0]}`);
				return DiscordModules.React.createElement(TooltipWrapper, {
					...
					children: (props) => DiscordModules.React.createElement('div', Object.assign({
						tabindex: 0,
						className: (element && !DOMTools.hasClass(element, '_closed')) ? `${icons.iconWrapper} ${icons.clickable} ${icons.selected}` : `${icons.iconWrapper} ${icons.clickable}`,
						// element check needed here, otherwise switching out of Server Discovery crashes Discord
						role: 'button'
					}, props),
						DiscordModules.React.createElement('svg', {
							name: 'ChannelButton',
